...
namespace Inflectra.KronoDesk.Service.Email.UI
{
	...
	public partial class MainWindow : Window
	{
		...
		private void AddOrUpdateEmailAccount(AccountDetails emailAcct)
		{
			if (emailAcct != null && emailAcct.AccountID.HasValue)
			{
				...
				{
					AccountDetails exisAcct = this.settings.Email_Accounts.Where(aps => aps.AccountID == emailAcct.AccountID).SingleOrDefault();
					if (exisAcct != null)
					{
				...
				else
				{
					...
					{
						AccountDetails highestAcct = this.settings.Email_Accounts.OrderBy(em => em.AccountID).ToList()[this.settings.Email_Accounts.Count - 1];
