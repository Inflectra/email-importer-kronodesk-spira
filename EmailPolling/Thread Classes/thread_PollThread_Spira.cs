...
using System.IO;
using Inflectra.KronoDesk.Service.Email.Service.Helper_Classes;
							...
							if (userCnt == 1)
							{
								...
								try
								{
									...
									switch (artType)
									{
										...
										case ArtifactTypeEnum.Incident:
                                            comment = clientAppl.Incident_AddComments(new List<RemoteComment>() { comment }).FirstOrDefault();
											break;
                                    ...
                                    if (comment != null && comment.CommentId.HasValue)
                                    {
                                        ...
                                        try
                                        {
                                            foreach (MessagePart attach in msg.FindAllAttachments().Where(aa => aa.IsMultiPart == false && aa.IsText == false))
                                            {
                                                ...
                                                newDoc.AttachedArtifacts = new List<RemoteLinkedArtifact>() { artifactLink };
