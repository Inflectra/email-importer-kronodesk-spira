...
namespace Inflectra.KronoDesk.Service.Email.Service
{
	...
	{
		private void processKronoAccount(Pop3Client clientPOP3, EmailAccount account, ApplicationSystem appServer)
		{
			...
			foreach (Message msg in newMsgs)
			{
				try
				{
						...
						msg.Headers.From.Address.ToLowerInvariant().Trim() != account.AccountEmail.ToLowerInvariant().Trim())
					{
						...
						if (this.doesMessageClearFilters(msg, out filterMsg))
						{
							...
							{
								tktId = this.retrieveTicketId(msg.Headers.UnknownHeaders[Common.MESSAGEHEADER_KRONO_TICKET]);
							}

							if (!tktId.HasValue)
							{
								Regex rgx = new Regex(@"\[[a-zA-Z]{2}[\:-][0-9]+\]", RegexOptions.IgnoreCase | RegexOptions.Singleline);
								tktId = this.retrieveTicketId(rgx.Match(msg.Headers.Subject).Value);
							}

							//Get users that this email is from..
							List<RemoteUser> usrFrom = clientAppl.User_RetrieveByEmailAddress(msg.Headers.From.MailAddress.Address);
							RemoteUser usr = null;
							...
							if (tktId.HasValue)
							{
								try
								{
									...
									if (!add3rd)
									{
										...
										newTktNote.AuthorId = usr.UserId;
										newTktNote.TicketId = tktId.Value;
										newTktNote.Text = this.getTextFromMessage(msg, true, true);
										...
										{
											this._eventLog.WriteMessage("Error adding note to message. Adding it as third-party.");
											add3rd = true;
										}

										if (!add3rd)
										{
											this._eventLog.WriteMessage("Note #" + newTktNote.NoteId.ToSafeString() + " added to Ticket #" + newTktNote.TicketId.ToSafeString() + " created from message " + msg.MessageLogID + ".", System.Diagnostics.EventLogEntryType.Information);
									...
									if (add3rd)
									{
										try
										{
											...
											RemoteDocument rawMsg = this.getMsgAsAttachment(msg, tktId.Value, null, "3rd Party Email:" + Environment.NewLine + strDescDetails);
											...
											this._eventLog.WriteMessage("Message from 3rd party to ticket. Message " + msg.MessageLogID + " saved as Attachment #" + rawMsg.AttachmentId.ToSafeString() + " & Note #" + newTktNote.NoteId + " added.", System.Diagnostics.EventLogEntryType.Information);
										...
										{
											this._eventLog.WriteMessage(METHOD, ex, "Error adding 3rd party email to ticket from message " + msg.MessageLogID);
										}
									}
								}
								catch (Exception ex)
								{
									this._eventLog.WriteMessage(METHOD, ex, "Error adding note to ticket from message " + msg.MessageLogID + ".");
									break;
								}
							}
							else
							{
								try
								{
									...
									if (usrFrom.Count == 0)
									{
										...
										Dictionary<string, string> names = this.getFullNameFromAddress(msg.Headers.From);
										...
										usr = clientAppl.User_Create(newUser, "abcdefghijkLMNOPQ1", "Registration Code", Membership.GeneratePassword(10, 0), new List<string>());
										this._eventLog.WriteMessage("Temporary user '" + newUser.Login + "' created, user ID#" + usr.UserId.ToSafeString(), System.Diagnostics.EventLogEntryType.Information);
									}
									else
									{
										//TODO: Find closest name. Should also check for locked or inactive users.
										usr = usrFrom[0];
									}
									...
									newTicket.Description = this.getTextFromMessage(msg, true);
									newTicket.OpenerId = usr.UserId;
									...
									if (account.UseRegexToMatch)
									{
										...
										if (regProd1.IsMatch(newTicket.Description) || regProd1.IsMatch(newTicket.Name))
										{
											...
											if (int.TryParse(matchNum, out prodNum))
											{
												...
												if (prod != null)
												{
													newTicket.ProductId = prod.ProductId.Value;
													this._eventLog.WriteMessage("Message " + msg.MessageLogID + " changed to Product '" + prod.Name + "' due to having [PR:xx] tag.", System.Diagnostics.EventLogEntryType.Information);
												}
											}
										}
										else
											...
											foreach (RemoteProduct prod in krnProds)
											{
												if (!string.IsNullOrWhiteSpace(prod.ProductToken))
												{
													...
													if (regProd2.IsMatch(newTicket.Description) || regProd2.IsMatch(newTicket.Name))
													{
														newTicket.ProductId = prod.ProductId.Value;
														foundToken = true;
														this._eventLog.WriteMessage("Message " + msg.MessageLogID + " changed to Product '" + prod.Name + "' due to having Product token '" + prod.ProductToken + "'", System.Diagnostics.EventLogEntryType.Information);
														break;
													}
												}
											}
									...
									this._eventLog.WriteMessage("Ticket #" + newTicket.TicketId.ToSafeString() + " created from message " + msg.MessageLogID + ".", System.Diagnostics.EventLogEntryType.Information);
								...
								{
									this._eventLog.WriteMessage(METHOD, ex, "Creating new ticket/user for message " + msg.MessageLogID);
									continue;
								}
							}
							...
							if (tktId.HasValue)
							{
								...
								try
								{
									...
									foreach (MessagePart attach in msg.FindAllAttachments().Where(aa => aa.IsMultiPart == false && aa.IsText == false))
									{
										...
										if (!doesExist)
										{
											...
											//Loggit.
											this._eventLog.WriteMessage("Attachment #" + newDoc.AttachmentId.ToSafeString() + " created from file '" + attach.FileName + "' for Ticket #" + tktId.ToSafeString() + " from message " + msg.MessageLogID + ".", System.Diagnostics.EventLogEntryType.Information);
										}
										else
											this._eventLog.WriteMessage("Attachment file '" + attach.FileName + "' not uploaded from message " + msg.MessageLogID + ", hash already exists in Ticket #" + tktId.ToSafeString() + ".", System.Diagnostics.EventLogEntryType.Information);
								...
								if (account.SaveRawEmailAsAttachment)
								{
									try
									{
										RemoteDocument rawMsg = this.getMsgAsAttachment(msg, tktId.Value, usr.UserId, null);
										rawMsg = clientAppl.Document_AddFile(rawMsg, msg.RawMessage);

										this._eventLog.WriteMessage("Message " + msg.MessageLogID + " saved as Attachment #" + rawMsg.AttachmentId.ToSafeString() + ".", System.Diagnostics.EventLogEntryType.Information);
									}
									catch (Exception ex)
									{
										this._eventLog.WriteMessage(METHOD, ex, "Saving message " + msg.MessageLogID + " as attachment:");
										break;
									}
								}
								...
								foreach (KeyValuePair<string, string> kvpURL in mappedURLs)
								{
									...
									else
									{
										...
										this._eventLog.WriteMessage("Both newTicket and newTktNote were null or not null while processing message " + msg.MessageLogID + ".", System.Diagnostics.EventLogEntryType.Warning);
									}
								}
								if (needsTktUpdating)
								{
									try
									{
										...
										this._eventLog.WriteTrace(METHOD, "Updating ticket description with attachment URLs for message " + msg.MessageLogID + ".");
										newTicket.LastUpdateDate = new DateTime(1976, 4, 26);
										clientAppl.Ticket_Update(newTicket);
									}
									catch (Exception ex)
									{
										this._eventLog.WriteMessage(METHOD, ex, "Trying to update ticket description for message " + msg.MessageLogID + ".");
									}
								}
								else if (needsNoteUpdating)
								{
									try
									{
										this._eventLog.WriteTrace(METHOD, "Updating note text with attachment URLs for message " + msg.MessageLogID + ".");
										clientAppl.Ticket_UpdateNote(newTktNote);
									}
									catch (Exception ex)
									{
										this._eventLog.WriteMessage(METHOD, ex, "Trying to update note text for message " + msg.MessageLogID + ".");
									}
								}
							}
						}
				...
				{
					this._eventLog.WriteMessage(METHOD, ex, "While importing message " + msg.MessageLogID);
				}
			}
			...
			{
				this._eventLog.WriteMessage("Trying to disconnect from server.", ex);
			}
		}
