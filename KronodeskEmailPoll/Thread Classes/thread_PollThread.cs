	...
	public partial class PollThread
	{
		...
		public PollThread()
		{
			...
			try
			{
				...
				this._configWatcher.Path = settingsDir;
				this._configWatcher.Filter = Common.SETTINGS_FILE;
				this._configWatcher.IncludeSubdirectories = false;
				this._configWatcher.NotifyFilter = NotifyFilters.Attributes | NotifyFilters.LastWrite | NotifyFilters.Size;
				this._configWatcher.Changed += new FileSystemEventHandler(configFile_Changed);
		...
		public void Start()
		{
			...
			this._timer = new Timer(this.TimerEvent, this, 0, this._settings.PollInterval * MILI);
		}
		...
		private void TimerEvent(object state)
		{
			...
			foreach (EmailAccount account in this._settings.Email_Accounts)
			{
				try
				{
					this._eventLog.WriteTrace(METHOD, "Running on account: '" + account.AccountEmail + "' (" + account.AccountID.ToString() + ")");

					//Get the appl's server definition to make sure it exists.
					ApplicationSystem serverDef = this._settings.Application_Servers.Where(ap => ap.ServerID == account.ApplicationServerID).SingleOrDefault();
						...
						(account.ProductOrProjectId.HasValue && account.ProductOrProjectId.Value > 0))  //And there's a defined default Product/Project.
					{
						...
						Pop3Client clientPOP3 = this.popConnect(account);

						if (serverDef.ServerType == ApplicationSystem.ApplServerTypeEnum.Spira)
						{
							this.processSpiraAccount(clientPOP3, account, serverDef);
						}
						else if (serverDef.ServerType == ApplicationSystem.ApplServerTypeEnum.Krono)
						{
							this.processKronoAccount(clientPOP3, account, serverDef);
						}
					}
					else
					{
						...
						this._eventLog.WriteMessage(msg, EventLogEntryType.Error);
					}
				}
				catch (Exception ex)
				{
		...
		private dynamic createApplicaitonClient(ApplicationSystem applicationSystem, EmailAccount emailSystem)
		{
			const string METHOD = CLASS + "createApplicaitonClient()";
			this._eventLog.EntryLog(METHOD);
			...
			if (applicationSystem.ServerType == ApplicationSystem.ApplServerTypeEnum.Spira)
			{
				...
				//Connect.
				this._eventLog.WriteTrace(METHOD, "Logging into Spira Client.");
				Settings.SpiraClient.ImportExportClient spiraClient = (Settings.SpiraClient.ImportExportClient)clientApp;
				if (spiraClient.Connection_Authenticate2(applicationSystem.UserID, applicationSystem.UserPassword, Common.APP_NAME))
				{
					this._eventLog.WriteTrace(METHOD, "Selecting project in Spira Client.");
			...
			else
			{
				...
				//Connect.
				this._eventLog.WriteTrace(METHOD, "Logging into Krono Client.");
				Settings.KronoClient.ImportExportClient kronoClient = (Settings.KronoClient.ImportExportClient)clientApp;
