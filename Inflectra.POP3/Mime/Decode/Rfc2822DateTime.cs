...
namespace Inflectra.POP3.Mime.Decode
{
	/// <summary>
	...
	{
		private const string CLASS_NAME = "Inflectra.POP3.Mime.Decode.Rfc2822DateTime::";

		/// <summary>Converts a string in RFC 2822 format into a <see cref="DateTime"/> object</summary>
		/// <param name="inputDate">The date to convert</param>
		/// <returns>A valid <see cref="DateTime"/> object, which represents the same time as the string that was converted. 
		/// If <paramref name="inputDate"/> is not a valid date representation, then <see cref="DateTime.MinValue"/> is returned.</returns>
		...
		{
			const string METHOD_NAME = CLASS_NAME + "AdjustTimezone";
		...
		{
			const string METHOD_NAME = CLASS_NAME + "ExtractDateTime()";
		...
		{
			const string METHOD = CLASS_NAME + "ValidateDayNameIfAny()";
			...
			if (dateInput.Length >= 4 && dateInput[3] == ',')
			{
				...
				{
					//Logger.LogMessage("Day-name does not correspond to the weekday of the date: " + dateInput, METHOD);
				}
			}
