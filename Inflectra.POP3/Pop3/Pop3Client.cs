...
using Inflectra.POP3.Exceptions;
using Inflectra.POP3;
using Inflectra.POP3.Common;

namespace Inflectra.POP3
{
	/// <summary>
	...
	{
		private const string CLASS = "Inflectra.POP3.Pop3Client::";
		...
		/// <summary>Connect to the server using user supplied stream</summary>
		...
		public void Connect(Stream stream)
		{
			...
			catch (Exception ex)
			{
				...
				throw new PopServerNotAvailableException("Server is not available", ex);
			}
		}
		...
		{
			const string METHOD = CLASS + "Authenticate()";
			...
			catch (PopServerException ex)
			{
				...
				throw new Exception("Problem logging in using method " + authenticationMethod + ". Server response was: " + LastServerResponse, ex);
			}
		...
		private static void CheckFailedLoginServerResponse(string serverErrorResponse, PopServerException e)
		{
			...
			{
				throw new Exception("Authentication: maildrop is locked or in-use.", e);
			}

			// Specifies that there must go some time between logins
			if (upper.Contains("[LOGIN-DELAY]"))
			{
				throw new Exception("Authentication: Delay between logins not reached.", e);
			}
		}
		#endregion
	}
