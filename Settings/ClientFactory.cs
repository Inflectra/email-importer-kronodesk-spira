...
{
	/// <summary>
	/// Factory for creating and configuring new Spira and KronoDesk SOAP API endpoints
	/// </summary>
	public static class ClientFactory
	{
        public static string SPIRA_API = "/Services/v5_0/SoapService.svc";
		public static string KRONO_API = "/Services/v3_0/SoapService.svc";
        ...
        public static SpiraClient.SoapServiceClient CreateClient_Spira(Uri fullUri)
		{
            ...
            SpiraClient.SoapServiceClient spiraImportExport = new SpiraClient.SoapServiceClient(httpBinding, endpointAddress);
		...
		/// <returns>The client class</returns>
		/// <remarks>We need to do this in code because the app.config file is not available in VSTO</remarks>
		public static KronoClient.SoapServiceClient CreateClient_Krono(Uri fullUri)
		{
			BasicHttpBinding httpBinding = ClientFactory.createBinding(fullUri.Scheme);

			//Create the new client with endpoint and HTTP Binding
			EndpointAddress endpointAddress = new EndpointAddress(fullUri.AbsoluteUri);
            KronoClient.SoapServiceClient kronoImportExport = new KronoClient.SoapServiceClient(httpBinding, endpointAddress);
